...
namespace
{
	...
	inline GpuWorldVertex ToGpuWorldVertex(const ProjectionVertex& vertex)
	{
	...
	void DumpComposedWorldFrame(uint8_t* palette, uint8_t* screenBuffer)
	{
		...
		// difference can be traced to a table row instead of guessed from RGB.
		{
			...
			{
				fwrite(ColourLookupTable_F6EE0, 1, sizeof(ColourLookupTable_F6EE0), tables);
				fclose(tables);
			}
		}
		...
		// comparison can tell a wrong texel from a wrong shade level.
		{
			...
			{
				fwrite(ColourLookupTable_F6EE0, 1, 16384 + 65536, table);
				fclose(table);
			}
		}
		Logger->info(
...
static bool TrySubmitBigWorldSpriteToGpu()
{
	...
	const int mode = str_F2C20ar.dword0x01_rotIdx;
	// Mode 8 is the shadow pass.  The CPU blit of this path has no case for it
	// - neither has the original's - so big sprites never had a shadow.  The
	// GPU could draw one (same shader branch as the normal path), but that
	// changes the look of the original, so the pass is consumed here as a
	// deliberate no-op instead of being handed back to the CPU blit.
	if (mode == 8)
	{
		return true;
	}
